...
public class ArticlesListFromJsonExtractorTest {
	...
	public static final String TWO_ARTICLES_JSON =
	...
	public void shouldExtractTwoArticles() {
		...
		// When
		final List<Article> articles = extractor.extractArticlesData(TWO_ARTICLES_JSON);

		// Then
		assertThat(articles.size() == 2);

		final Article articleOne = articles.get(0);
		...
		final Article articleTwo = articles.get(1);
	...
	public void shouldExtractStartIndexes() {
		...
		// When
		final List<Integer> indexes = ArticlesListFromJsonExtractor.getArticlesStartIndexes(TWO_ARTICLES_JSON);
	...
	public void shouldNotExtractAnythingOnStringWithoutPatternMatch() {
		...
		// Given
		final String jsonString = "{\"status\":1,\"complete\":1}";

		// When
		final List<Integer> indexes = ArticlesListFromJsonExtractor.getArticlesStartIndexes(jsonString);
	...
	public void shouldExtractTwoStringsWithArticles() {
		...
		// When
		final List<String> articleStrings = ArticlesListFromJsonExtractor.extractArticleStrings(TWO_ARTICLES_JSON, Lists.newArrayList(45, 887));
